...
export class DwtComponent implements OnInit, OnDestroy {
  ...
  submitted = false;
  b64Data: string = '';
  isReadBarcode: boolean = false;

  constructor(protected dwtService: DwtService, private readonly notificationService: NotificationService, private modalService: NgbModal, private readonly documentmanagerService: DocumentmanagerService, private readonly documentManagerComponent: DocumentManagerComponent, private router: Router, private readonly commonService: CommonMethodService, private readonly storageService: StorageService, private formBuilder: FormBuilder) {
    this.initDWT();
  ...
  ngOnInit() {
    ...
    if (this.data === 'AutoRouteV2') {
    this.commonService.getObserveFlag.subscribe((res) => {
      this.saveOptions.multiPage = JSON.parse(res).mergePage ? JSON.parse(res).mergePage:false;
      this.isReadBarcode = JSON.parse(res).readbarcode ? JSON.parse(res).readbarcode:false;
    });
  ...
  showMessage(msg: string) {
    ...
    let error: any = { message: msg, statusText: 'Error', status: 400 }
    if ((error.message.includes('-1023'))) {
      error = 'The device went offline prior to or during this operation';
    }
    if ((error.message.includes('-1032'))) {
  ...
  handleMultiPageCheck() {
    if (this.DWObject?.SelectedImagesIndices) {
  ...
  save() {
    ...
    if (this.data === 'AutoRouteV2') {
      ...
      if (this.saveOptions.multiPage) {
        ...
        ////this.DWObject.LoadImage("@D:\PRECISEMRINEW\DEVELOPMENT\CODEBASE\Branches\BugFixing\PreciseMRI.Web\src\assets\dwt");
        let Doc: any = this.saveOptions.outPutType
        let docTypeBody: any = {
          'filename': this.saveOptions.fileName,
          'files': encodeURIComponent(this.saveOptions.fileName),
          'docTypeId': Doc.DocId,
          'docType': Doc.DocType,
          'referreId': this.data,
        ...
        let body: any = JSON.stringify(docTypeBody);
        let selectedIndices = this.DWObject.SelectedImagesIndices;
        this.dwtService.uploadToServerAutoRoute(selectedIndices, this.getImageType(this.saveOptions.outPutFormat), this.saveOptions.fileName, body)
          .then(result => {
            this.successNotification(result);
            this.patientList.push(result.responsString)

            /*  this.saveResults.uploadedFiles.push(result);*/
            this.clearMessage();
          }, err => this.showMessage(err));
      }
      else {
        ...
        for (let i = 0; i < count; i++) {
          let Doc: any = this.saveOptions.outPutType
          let docTypeBody: any = {
            'filename': this.saveOptions.fileName + "_" + (i + 1),
            'files': encodeURIComponent(this.saveOptions.fileName),
            'docTypeId': Doc.DocId,
            'docType': Doc.DocType,
            'referreId': this.data,
          ...
          this.dwtService.uploadToServerAutoRoute([i], this.getImageType(this.saveOptions.outPutFormat), this.saveOptions.fileName + "_" + (i + 1), body)
            .then(result => {
              this.successNotification(result);
              this.patientList.push(result.responsString)
              /*  this.saveResults.uploadedFiles.push(result);*/
    ...
    if (this.saveOptions.upload) {
      if (this.saveOptions.multiPage) {
        ...
        let body: any = JSON.stringify(docTypeBody);
        let selectedIndices = this.DWObject.SelectedImagesIndices;
        this.dwtService.uploadToServer(selectedIndices, this.getImageType(this.saveOptions.outPutFormat), this.saveOptions.fileName, body)
          .then(result => {
            this.successNotification(result);
            this.saveResults.uploadedFiles.push(result);
            this.clearMessage();

            this.commonService.sendlist(this.data);
        ...
        for (let i = 0; i < count; i++) {
          ...
          this.dwtService.uploadToServer([i], this.getImageType(this.saveOptions.outPutFormat), this.saveOptions.fileName + "_" + (i + 1), body)
            .then(result => {
              this.successNotification(result);
              this.saveResults.uploadedFiles.push(result);
              this.clearMessage();
  ...
  uploadToEpic() {
    ...
    // else {
    if (!this.saveOptions.fileName || !this.saveOptions.outPutType) {
      this.modelValue = '';
      return;
    }
    else {
  ...
  saveToMyPc() {
    this.saveOptions.upload = false;
    // this.saveOptions.outPutType = 'server'
    if (this.data === 'AutoRouteV2') {
      this.submitted = true;
  ...
  successNotification(data: any) {
    this.notificationService.showNotification({
      alertHeader: 'Success',
      alertMessage: 'Uploladed Successfully',
      alertType: 200
    });
  }
    ...
    if (this.data === 'AutoRouteV2') {
      ...
      if (this.saveOptions.upload) {
        if (this.saveOptions.multiPage) {
          ...
          let Doc: any = this.saveOptions.outPutType
          let docTypeBody: any = {
            'filename': this.saveOptions.fileName ? this.saveOptions.fileName : 'FileName',
            'files': encodeURIComponent(this.saveOptions.fileName),
            'docTypeId': Doc.DocId,
            'docType': Doc.DocType,
            'referreId': this.data,
            'owner': this.storageService.user.UserId,
            'fromPage': 'Dynamsoft',
            'isReadBarcode': this.isReadBarcode
          }
          let body: any = JSON.stringify(docTypeBody);
          let selectedIndices = this.DWObject.SelectedImagesIndices;
          this.dwtService.uploadToServerAutoRoute(selectedIndices, this.getImageType(this.saveOptions.outPutFormat), this.saveOptions.fileName, body)
            .then(result => {
              ...
              /*  this.saveResults.uploadedFiles.push(result);*/
              this.clearMessage();
        /*      $('.sureshDocx').trigger('click')*/
            }, err => this.showMessage(err));
        }
        else {
          let count = this.DWObject.HowManyImagesInBuffer;
          this.saveOptions.fileName = this.saveOptions.fileName ? this.saveOptions.fileName : 'FileName';
          for (let i = 0; i < count; i++) {
            let Doc: any = this.saveOptions.outPutType
            let docTypeBody: any = {
              'filename': this.saveOptions.fileName + "_" + (i + 1),

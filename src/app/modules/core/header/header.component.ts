...
export class HeaderComponent implements OnInit {
  @ViewChild('hiddenDocManager', { static: false })
  hiddenDocManager: ElementRef;
  ...
  applyFilter(
    ...
    this.applyFilterBody = new TaskManagementApplyFilter(
      assignedBy,
      assignedTo,
      dueDate,
      label,
      savedSearch,
      status,
      this._storageService.user.UserId
    );
    this.taskManagementService
      .taskManagementApplyFilter(true, this.applyFilterBody)
      .subscribe(
        (res) => {
  ...
  showPreScreeningPopup() {
    const modalReff = this._modalService.open(
  ...
  showDocManager(patientId: string) {
    patientId = patientId.toLowerCase().includes('pre')
      ? patientId
      : 'PRE' + patientId;
    {
      this._commonMethodService.checkPatientExist(true, patientId).subscribe(
        (res) => {
  ...
  getPermission() {
    ...
    let docManagertemp = this.headerPopupList.find(
      (x) => x.PageTitle == 'DOCUMENT MANAGER'
    );
    this.docManager =
      docManagertemp && docManagertemp.PageTitle
        ? docManagertemp.PageTitle
        : '';
    let taskManagertemp = this.headerPopupList.find(
      (x) => x.PageTitle == 'TASK MANAGER'
    );
    this.taskManager =
      taskManagertemp && taskManagertemp.PageTitle
        ? taskManagertemp.PageTitle
        : '';
    let creteAlerttemp = this.headerPopupList.find(
      (x) => x.PageTitle == 'CREATEALERT'
    );
    this.createAlert =
      creteAlerttemp && creteAlerttemp.PageTitle
        ? creteAlerttemp.PageTitle
        : '';
    let preScreentemp = this.headerPopupList.find(
      (x) => x.PageTitle == 'PRE-SCREENING QUESTIONS'
    );
    this.preScreenQuestions =
      preScreentemp && preScreentemp.PageTitle ? preScreentemp.PageTitle : '';
    let autoRoutetemp = this.headerPopupList.find(
      (x) => x.PageTitle == 'AUTOROUTEV2'
    );
    this.autoRouteV2 =
      autoRoutetemp && autoRoutetemp.PageTitle ? autoRoutetemp.PageTitle : '';

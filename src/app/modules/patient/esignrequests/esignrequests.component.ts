...
@Component({
  selector: 'app-esignrequests',
  templateUrl: './esignrequests.component.html',
  styleUrls: ['./esignrequests.component.css'],
})
export class EsignrequestsComponent implements OnInit {
  ...
  selectedInputFont = '';
  selectedLanguageText = 'Elija fuente';
  typeSignature = '';
  selectedModOfSignature = 'Type';
  btnStartVal: any = 'Por favor complete 2 campos obligatorios.';
  eSignFromImage: any;
  selectedFile: any;
  fileName: any;
  pFirstSign: any;
  ...
  downloadSignFile() {
    this.patientService
      .download(this.patientid, this.Name, 'esignrequests')
      .subscribe(
        (res) => {
          if (res.responseCode === 200) {
      ...
      if (this.completedFirstSignature === true &&
        ...
        if (this.selectedModOfSignature === 'Draw') {
          if (this.f.valid) {
            ...
            this.patientService
              .insertESignatureLien(data, 'esignrequests')
              .subscribe(
                (res) => {
  ...
  openlien() {
    if (this.agreeCheck) {
      ...
      this.CodeErrorNotification(
        'Acepte usar registros electrónicos y firma.'
      );
    }
  }
  ...
  previewAndSave(type: number) {
    this.patientService
      .printPreview(this.patientid, this.Name, 'esignrequests')
      .subscribe(
        (res) => {
          if (res.responseCode === 200) {
            let response = JSON.parse(res.response);

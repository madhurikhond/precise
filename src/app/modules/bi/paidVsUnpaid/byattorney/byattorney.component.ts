...
@Component({
  selector: 'app-byattorney',
  templateUrl: './byattorney.component.html',
  styleUrls: ['./byattorney.component.css']
})
export class ByattorneyComponent implements OnInit {
  totalRecords: number
  attorneyPaidUnpaidStudiesList:any =[]
  ...
  ngOnInit(): void {
   ...
   this.getAttorneyPaidUnpaidStudies();
  }
  getAttorneyPaidUnpaidStudies() {
    ...
    if (res.response != null && res.response.length > 0) {   
      this.attorneyPaidUnpaidStudiesList=res.response;  
    }
    else{

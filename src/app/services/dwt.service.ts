...
import { DocumentmanagerService } from './document-manager-service/document.manager.service';
...
export class DwtService {
  ...
  /**
   * bWASM is for WASM mode which is opposeddwenvi to the Service mode.
  ...
  constructor(private readonly notificationService: NotificationService, private readonly _httpService: HttpService) {
    this.apiUrl = `${environment.baseUrl}/v${environment.currentVersion}/`;
    this.getDynamProductKey().subscribe((res) => {
      if (res.response != null) {
        this.productKey = res.response;
    ...
    let outputResults = () => {
      ...
      if (this.barcodeResults.length === 0) {
        ...
        for (let i = 0; i < allBarcodeResults.length; ++i) {
          ...
          let newobject = {
            Files: JSON.stringify(resultString)
          }
          this.uploadFileToAPI(newobject)
        }
  ...
  uploadFileservice(content) {
    return this._httpService.post(`DocumentManager/UploadFileBarCode`, content, this.showGlobalLoader, true).pipe(
      map((res: ApiResponse) => res)
    );
  }
  uploadFileToAPI(content) {
    this.uploadFileservice(content).subscribe((res) => {
      if (res.response != null) {
   ...
   * @param fileName Specify the file name.
   */
  uploadToServer(indices: number[], type: DynamsoftEnumsDWT.EnumDWT_ImageType, fileName: string, docTypeBody: any): Promise<any> {
    return new Promise((res, rej) => {
      fileName = fileName + this.getExtension(type);
      let url = "", savedDir = "";
      if (environment.Dynamsoft.uploadTargetURL !== "") {
        ...
        let _strPort = 2020;
        let strActionPage = "/uploadfileDynamsoft";
        savedDir = protocol + window.location.hostname + ":" + _strPort + "/uploaded/";
        url = this.apiUrl + "DocumentManager/UploadFile";

        fileName = JSON.stringify(docTypeBody);
      }
      ...
      this._DWObject.HTTPUpload(
        ...
        (errCode, errString, responseStr) => {
          if (responseStr !== "") {
            //rej("Uploaded Succcessfully")
            res({ name: 'Uploaded Succcessfully', url: "Uploaded Succcessfully" });
  ...
  uploadToServerAutoRoute(indices: number[], type: DynamsoftEnumsDWT.EnumDWT_ImageType, fileName: string, docTypeBody: any): Promise<any> {
    return new Promise((res, rej) => {
      fileName = fileName + this.getExtension(type);
      let url = "", savedDir = "";
      if (environment.Dynamsoft.uploadTargetURL !== "") {
        ...
        let strActionPage = "/uploadfileDynamsoft";
        savedDir = protocol + window.location.hostname + ":" + _strPort + "/uploaded/";
        url = this.apiUrl + "DocumentManager/UploadFileAutoRouteV2";
      ...
      this._DWObject.HTTPUpload(
        ...
        (errCode, errString, responseStr) => {
          if (responseStr !== "") {
            //rej("Uploaded Succcessfully")
            res({ type: 'httpResponse', responsString: JSON.parse(responseStr).response });
            //  this.generalSubject.next({ type: "httpResponse", responsString: responseStr });
          }
          if (errString == "") {
            rej(errString);
  ...
  getDynamProductKey(showGlobalLoader: boolean = true) {
    return this._httpService.get(`DocumentManager/GetDynamProductKey`, showGlobalLoader).pipe(
      map((res: ApiResponse) => res)
    );
  }
}
